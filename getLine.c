...
ssize_t get_input(info_t *info)
{
	...
	if (len)
	{
		...
		*buf_p = p; /* pass back the pointer to current command position */
		return (_strlen(p)); /* It returns length of current command */
	}

	*buf_p = buf; /* else not a chain, pass back the buffer from _getline() */
	return (r); /* It returns length of buffer from _getline() */
}
 ...
 * @ptr: It's the address of pointer to buffer, preallocated or NULL
 * @length: It's the size of preallocated ptr buffer if not NULL
 *
 * Return: s
 */
